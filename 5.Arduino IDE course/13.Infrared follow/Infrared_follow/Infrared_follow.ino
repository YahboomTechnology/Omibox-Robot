...
* @file         Infraed_follow.c
* @author       xiaozhen
* @version      V1.0
* @date         2018.10.18
* @brief        Infraed_follow
* @details
* @par History  
*
*/
...
void setup()
{
   ...
   Serial.begin(115200); //Set the serial port baud rate, which will be used when printing data with the serial port.
   Serial.println("1234");
   pwm.begin();
...
{ 
    ...
    {
       run(800); //when obstacles are detected on both the left and right sides, Call run function 
    }
    else if (LeftSensorValue == HIGH && RightSensorValue == LOW)
    {
      spin_left(650);  //An obstacle was detected on the left, robot car spin left
    }
    else if (RightSensorValue == HIGH && LeftSensorValue == LOW)
    {
      spin_right(650); //An obstacle was detected on the right, robot car spin right
    }
    else
    {
      brake();
