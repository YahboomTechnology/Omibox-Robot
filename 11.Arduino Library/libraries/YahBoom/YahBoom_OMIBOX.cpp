...
const PROGMEM unsigned char Heart[16] = {
	0x6,0x30,0xf,0x78,0xf,0xf8,0xf,0xf8
	,0x7,0xf0,0x3,0xe0,0x1,0xc0,0x0,0x80};
//三角形
...
void YahBoom_OMIBOX_Matrix::roll(int time0,uint8_t t, uint8_t *pr1)//将通过移位的方式让点阵显示的字符可以左右移动
{
	int i,j,m;
	...
	if(Num_ofword<16)//数组个数小于16的也就是一屏就能全部显示的字符串
	{
	...
	if(Num_ofword>15)//数组个数大于16的
	{
		for(i=0;i<Num_ofword+1;i++)
		{
			if(t==1)//字符左移的方式
			{
				for(j=0;j<8;j++)
				{
					lc.setColumn(0, j, a[j]); //显示0号点阵
				}

				for(j=0;j<8;j++)
				{
					lc.setColumn(1, j, a[j+8]); //显示1号点阵
				}
				m=a[0];
				for(j=0;j<Num_ofword;j++)
				{
			...
			if(t==2)//字符右移
			{
				for(j=0;j<8;j++)
				{
					lc.setColumn(0, j, a[Num_ofword-17+j]); //显示0号点阵
				}
				for(j=0;j<8;j++)
				{
					lc.setColumn(1, j, a[Num_ofword-9+j]); //显示1号点阵
				}
					m=a[Num_ofword-1];
...
void YahBoom_OMIBOX_Matrix::picture( uint8_t *pr1,uint8_t t,int time0)//显示图片
{
    int i,j,m;
    int a[16],c[16];
...
void YahBoom_OMIBOX_Matrix::Move_picture(uint8_t x,uint16_t time)
{
	if(x==1)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)nictation0);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)nictation1);	
		delay(time);
	}
	if(x==2)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)heart0);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)heart1);	
		delay(time);
	}
	if(x==3)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)funny1);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)funny2);	
		delay(time);
	}
	if(x==4)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)doubt1);	
		delay(time*2);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)doubt2);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)doubt3);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)doubt4);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)doubt5);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)doubt6);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)doubt7);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)doubt8);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)doubt9);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)doubt10);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)doubt11);	
		delay(time*2);
	}
	if(x==5)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)mischievous_ghost1);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)mischievous_ghost2);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)mischievous_ghost3);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)mischievous_ghost4);	
		delay(time);
	}
	if(x==6)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)crying_wolf1);	
		delay(time);
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)crying_wolf2);	
		delay(time);
	}
...
void YahBoom_OMIBOX_Matrix::Static_picture(uint8_t x)
{
	if(x==1)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)Heart);	
	}
	if(x==2)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)Triangle );	
	}
	if(x==3)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)House );	
	}
	if(x==4)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)Boat );	
	}	
	if(x==5)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)Music );	
	}
	if(x==6)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)Umbrella );	
	}	
	if(x==7)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)Right );	
	}	
	if(x==8)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)Error );	
	}
	if(x==9)
	{
		YahBoom_OMIBOX_Matrix_ShowIcon((byte *)Rectangle );	
	}	
}
...
void YahBoom_OMIBOX_Buzzer::birthday()
{
	int j;
...
void YahBoom_OMIBOX_Buzzer::childhood()
{
	int j;
...
void YahBoom_OMIBOX_Buzzer::fenshuajiang()
{
	int j;
	for(j=0;j<48;j++)
	{
...
void YahBoom_OMIBOX_Buzzer::wanghouyusheng()
{
	int j;
	for(j=0;j<75;j++)
	{
...
void YahBoom_OMIBOX_Buzzer::huanlesong()
{
	int j;
	for(j=0;j<70;j++)
	{
...
void YahBoom_OMIBOX_Buzzer::yuemayangbian()
{
	int j;
	for(j=0;j<48;j++)
	{
...
void YahBoom_OMIBOX_Buzzer::guangdongaiqinggushi()
{
	int j;
	for(j=0;j<39;j++)
	{
...
void YahBoom_OMIBOX_Buzzer::shishangzhiyoumamahao()
{
	int j;
	for(j=0;j<34;j++)
	{
...
void YahBoom_OMIBOX_Buzzer::rangwomendangqishuangjiang()
{
	int j;
	for(j=0;j<67;j++)
	{
...
void YahBoom_OMIBOX_Buzzer::xinnianhao()
{
	int j;
	for(j=0;j<30;j++)
	{
...
void YahBoom_OMIBOX_Buzzer::xiaoxingxing()
{
	int j;
	for(j=0;j<42;j++)
	{
...
void YahBoom_OMIBOX_Buzzer::liangzhilaohu()
{
	int j;
	for(j=0;j<32;j++)
	{
...
void YahBoom_OMIBOX_Buzzer::music_Play(uint8_t v_song, uint8_t v_repeat)
{
	switch(v_song)
	{ 
		case enBirthday:
...
void YahBoom_OMIBOX_IRrecv::YahBoom_OMIBOX_IRrecv_Init()
{
	...
	pinMode(A3, INPUT_PULLUP);     //将A3号管脚设置为输入并且内部上拉模式2
...
void YahBoom_OMIBOX::Init()
{
	...
	YahBoom_OMIBOX_RGBLED cRGB(16, 5);
	...
	free(picTemp);
	cMatrix.YahBoom_OMIBOX_Matrix_ShowIcon(picBuffer);	
}